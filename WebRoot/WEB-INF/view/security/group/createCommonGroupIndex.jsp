...
<smart:body>
					...
					<smart:form id="saveForm">
						<smart:gridRow>
							<smart:gridColumn colPart="12">
								<smart:textInput labelName="用户群组代码：" autocomplete="off" placeholder="请输入用户群组代码" verify="required" name="code" isNotNull="true">
								</smart:textInput>
							</smart:gridColumn>
						</smart:gridRow>
						<smart:gridRow>
							<smart:gridColumn colPart="12">
								<smart:textInput labelName="用户群组名称：" autocomplete="off" placeholder="请输入用户群组名称" verify="required" name="name" isNotNull="true">
								</smart:textInput>
							</smart:gridColumn>
						</smart:gridRow>
						<smart:gridRow>
	...
	<smart:scriptHead models="form,layer,element,linkSelect">
		<smart:initLinkSelect id="groupTypeId" name="type" tips="请选择用户群组类型" verify="required" url="dictquery/sub/code/SECURITY_GROUP_TYPE"/>
		var index = parent.layer.getFrameIndex(window.name);
		form.on('submit(smart-submit)', function(obj) {
			var mask = layer.load(1, {
			  	shade: [0.5,'#fff']
			});
			$.post('group/common/create/save',
				obj.field,
				function(result) {	
					layer.close(mask);
					if (result.success) {
