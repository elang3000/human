	...
	<smart:grid>
		<smart:gridRow>
			<blockquote class="layui-elem-quote">
				<div>所在区域： ${organTree.name}&nbsp;&nbsp;&nbsp;组织名称：${organNode.allName}</div>
			</blockquote>
		</smart:gridRow>
		<smart:fieldSet title = "组织权限查询" color="blue">
			<smart:gridRow>
				<smart:form id="searchForm">
					<input type="hidden" name="organNodeId" value="${organNode.id}"/>
					<input type="hidden" name="organTreeId" value="${organTree.id}"/>
					<smart:gridRow>
 		...
 		treeGrid.on('tool(treeTable)', function(obj){
    		...
    		if(obj.event === 'accredit'){
      			smart.confirm({
					title : '授权',
					message : '确认授予相关权限？',
					url:'security/authc/organ/accredit/do',
					headers : {
   			...
   			if(obj.event === 'antiAccredit'){
      			smart.confirm({
					title : '取消授权',
					message : '取消授予相关权限？',
					url:'security/authc/organ/accredit/undo',
					headers : {
				        'Accept': 'application/json',
				        'Content-Type': 'application/json'
				    },
   			...
   			if(obj.event === 'antiRevoke'){
      			smart.confirm({
					title : '取消禁止',
					message:'取消禁止相关权限？',
					url:'security/authc/organ/accredit/unrevoke',
					headers : {
				        'Accept': 'application/json',
				        'Content-Type': 'application/json'
				    },
   			...
   			if(obj.event === 'revoke'){
      			smart.confirm({
					title : '禁止',
					message:'确认禁止相关权限？',
					url:'security/authc/organ/accredit/revoke',
					params:JSON.stringify(pArray),
					headers : {
				        'Accept': 'application/json',
				        'Content-Type': 'application/json'
