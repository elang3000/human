...
<smart:body>
	<smart:grid>
		<div class="layui-card">
			<div class="layui-card-body">
				<smart:fieldSet title="维护组织关系类型信息" color="blue" style="padding-top:1em;">
					<smart:form id="saveForm">
						<smart:textInput type="hidden" name="id" value="${type.id}"></smart:textInput>
						<smart:gridRow>
							<smart:gridColumn colPart="8">
								<smart:textInput labelName="关系类型编码：" value="${type.code}" autocomplete="off" placeholder="请输入组织关系类型编码" verify="required" name="code" isNotNull="true">
								</smart:textInput>
							</smart:gridColumn>
						</smart:gridRow>
						<smart:gridRow>
							<smart:gridColumn colPart="8">
								<smart:textInput labelName="关系类型名称：" value="${type.name}" autocomplete="off" placeholder="请组织关系类型名称" verify="required" name="name" isNotNull="true">
								</smart:textInput>
							</smart:gridColumn>
						</smart:gridRow>
	...
	<smart:scriptHead models="form,layer,element">
		...
		form.on('submit(smart-submit)', function(obj) {
			$.post('system/organ/relation/type/edit/save',
				obj.field,
				function(result) {	
					if (result.success) {
