...
<smart:body>
			...
			<div class="layui-card-body">
				<smart:fieldSet title="维护组织类型规则信息" color="blue" style="padding-top:1em;">
					<smart:form id="saveForm">
						...
						<smart:gridRow>
							<smart:gridColumn colPart="8">
								<smart:textInput labelName="类型规则编码：" autocomplete="off" placeholder="请输入组织类型规则编码" verify="required" name="code" isNotNull="true">
								</smart:textInput>
							</smart:gridColumn>
						</smart:gridRow>
						<smart:gridRow>
							<smart:gridColumn colPart="8">
								<smart:textInput labelName="类型规则名称：" autocomplete="off" placeholder="请输入组织类型规则名称" verify="required" name="name" isNotNull="true">
								</smart:textInput>
							</smart:gridColumn>
						</smart:gridRow>
						<smart:gridRow>
	...
	<smart:scriptHead models="form,layer,element,linkSelect">
		<smart:initLinkSelect id="superiorNodeTypeTag" name="superiorNodeTypeId" tips="请选择上级组织类型规则" verify="required" url="system/organ/node/type/query"/>
 		<smart:initLinkSelect id="subordinateNodeTypeTag" name="subordinateNodeTypeId" tips="请选择下级组织类型规则" verify="required" url="system/organ/node/type/query"/>
		var index = parent.layer.getFrameIndex(window.name);
		form.on('submit(smart-submit)', function(obj) {
			var mask = layer.load(1, {
			  	shade: [0.5,'#fff']
			});
			$.post('system/organ/tree/rule/create/save',
				obj.field,
				function(result) {	
					layer.close(mask);
					if (result.success) {
