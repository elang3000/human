...
<smart:body>
					...
					<smart:form id="saveForm">
						<smart:gridRow>
							<smart:gridColumn colPart="8">
								<smart:textInput labelName="关系类型编码：" autocomplete="off" placeholder="请输入组织关系类型编码" verify="required" name="code" isNotNull="true">
								</smart:textInput>
							</smart:gridColumn>
						</smart:gridRow>
						<smart:gridRow>
							<smart:gridColumn colPart="8">
								<smart:textInput labelName="关系类型名称：" autocomplete="off" placeholder="请输入组织关系类型名称" verify="required" name="name" isNotNull="true">
								</smart:textInput>
							</smart:gridColumn>
						</smart:gridRow>
	...
	<smart:scriptHead models="form,layer,element">
		...
		form.on('submit(smart-submit)', function(obj) {
			...
			$.post('system/organ/relation/type/create/save',
				obj.field,
				function(result) {	
					layer.close(mask);
					if (result.success) {
