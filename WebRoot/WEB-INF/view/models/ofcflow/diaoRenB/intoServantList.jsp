...
<smart:body>
	<smart:grid>
		<smart:card>
			<smart:cardBody>
				<smart:gridRow>
					<smart:title title="调入单位编制信息" style="margin-top: 5px;" color="blue" />
				</smart:gridRow>
				...
				<smart:gridRow>
					...
					<smart:gridColumn colPart="4">
						<smart:infoShowerLabel infoname="调任总人数 "
							infovalue="${d.sumNumber}"></smart:infoShowerLabel>
					</smart:gridColumn>
				</smart:gridRow>
				<smart:gridRow>
					<smart:gridColumn colPart="4">
						<smart:infoShowerLabel infoname="乡科级正职（领导）调任人数"
							infovalue="${d.plusKeLeaderNum}"></smart:infoShowerLabel>
					</smart:gridColumn>
					<smart:gridColumn colPart="4">
						<smart:infoShowerLabel infoname="乡科级正职（非领导）调任人数"
							infovalue="${d.plusKeNoLeaderNum}"></smart:infoShowerLabel>
					</smart:gridColumn>
				</smart:gridRow>
				<smart:gridRow>
					<smart:gridColumn colPart="4">
						<smart:infoShowerLabel infoname="乡科级副职（领导）调任人数"
							infovalue="${d.minusKeLeaderNum}"></smart:infoShowerLabel>
					</smart:gridColumn>
					<smart:gridColumn colPart="4">
						<smart:infoShowerLabel infoname="乡科级副职（非领导）调任人数"
							infovalue="${d.minusKeNoLeaderNum}"></smart:infoShowerLabel>
					</smart:gridColumn>
				</smart:gridRow>
				<smart:gridRow>
					<smart:gridColumn colPart="4">
						<smart:infoShowerLabel infoname="科员级调任人数 "
							infovalue="${d.clerkNumber}"></smart:infoShowerLabel>
					</smart:gridColumn>
					<smart:gridColumn colPart="4">
						<smart:infoShowerLabel infoname="办事员级调任人数"
					...
					<smart:gridColumn colPart="12" deviceType="md">
						<smart:buttonGroup container="true" align="right">
							<c:if test="${d.areaType=='2'}">
								<smart:button method="addPerson" size="sm" title="新增调任人员"
									theme="normal">
									<smart:icon icon="plus">&nbsp;新增调任人员</smart:icon>
								</smart:button>
							</c:if>
							<c:if test="${d.areaType=='1'}">
								<smart:button method="selectPerson" size="sm" title="选择调任人员"
									theme="normal">
						...
						<smart:table id="navigationList" doneCallBack="fixedCol"
							url="ofcflow/diaorenB/intoServantList?id=${d.id}"
							text="未获取到数据！">
							<tr>
								...
								<smart:tableItem field="enterReason" width=".1" sort="true">进入本单位原因</smart:tableItem>
		...
		<smart:tableScriptAction tableId="navigationList" checkbox="true"
			...
			edit : function(data) {
				smart.show({
					title : '编辑调入信息',
					size : 'full',
					url : "ofcflow/diaorenB/servantEdit?id="+data.data.id,
					scrollbar : false
				});
			}
			,delete : function(data) {
				smart.confirm({
					title:'提示',
					message:'您确定要删除这条记录？',
					url:'ofcflow/diaorenB/servantDelete',
			...
			,view : function(data) {
				smart.show({
					title : '查看调入信息',
					size : 'full',
					url : "ofcflow/diaorenB/servantView?id="+data.data.id,
					scrollbar : false
				});
			}
			,reload : function(){
		...
		<smart:buttonScriptAction>
			...
			selectPerson : function() {
			    smart.show({
					title : '选择人员',
					size : 'full',
					url : "orgInfo/selectServants?busClass=com.wondersgroup.human.bo.ofcflow.DiaoRenInto&createOrganNodeId=${d.sourceOrgan.id}&ids="
					+$("#servantIds").val()+"&names="+$("#servantNames").val()+"&personType=2&busParentClass=diaoRenIntoBatch",
					scrollbar : false,
				});
			}
			,addPerson : function(data) {
				smart.show({
					title : '新增调任人员',
					size : 'full',
					url : "ofcflow/diaorenB/servantEdit?batchId=${d.id}",
					scrollbar : false
				});
			}
			,goBack : function() {
 		...
 		form.on('submit(submit)', function (data) {//表单提交
 			...
 			smart.confirm({
				...
				<c:if test="${d.areaType eq '1' }">
					url:'ofcflow/diaorenB/operationFlow',
				</c:if>
		<c:if test="${d.areaType eq '2' }">
					url:'ofcflow/diaorenB/operationFlowOuter',
				</c:if>
				params : smart.json("#editForm"),
				callback : function(){
					parent.layui.table.reload('navigationList');
		...
		setInterval(function(){
		    if($('#servantIds').val()!=""){
		    	if($('#servantIds').val()==""){
			 	...
			 	var requestConfig = {};
				requestConfig.url = "ofcflow/diaorenB/savePeople";
