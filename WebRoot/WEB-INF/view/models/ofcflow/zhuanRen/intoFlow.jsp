				...
				<smart:form id="editForm">
					...
					<c:if test="${isFlow}">
						<c:if test="${d.status==6 }">
							...
							<smart:gridRow>
								<smart:gridColumn colPart="4">
									<smart:textInput type="hidden" name="zhuanRenTLBOutMgr.id" value="${o.id}"></smart:textInput>
									<smart:infoShowerLabel infoname="调往单位名称" infovalue="${o.gotoUnitName}"></smart:infoShowerLabel>
								</smart:gridColumn>
								<smart:gridColumn colPart="4">
									<smart:singleSelect initSelectedKey="${o.category.id}" name="zhuanRenTLBOutMgr.category.id"
								...
								<smart:gridColumn colPart="4">
									<smart:date labelName="调出本单位日期：" display="block" name="zhuanRenTLBOutMgr.outDate" id="outDate" value="${o.outDate}"></smart:date>
								</smart:gridColumn>
							</smart:gridRow>
							<smart:gridRow>
								<smart:gridColumn colPart="4">
									<smart:singleSelect labelName="提出调动类型：" name="zhuanRenTLBOutMgr.proposeType.id" display="block" url="dictquery/sub/code/DM039" isAddDefaltOption="true" initSelectedKey="${o.proposeType.id }"></smart:singleSelect>
								</smart:gridColumn>
								<smart:gridColumn colPart="8">
									<smart:continuousSelect labelName="调动原因：" inputName="zhuanRenTLBOutMgr.reasonType.id" codeTypeCode="DM015" inputVal="${o.reasonType.id}" valType="ID"/>
								</smart:gridColumn>
							</smart:gridRow>
							<smart:gridRow>
								<smart:gridColumn colPart="8">
									<smart:textarea labelName="调出备注：" name="zhuanRenTLBOutMgr.remark"  display="block">${o.remark}</smart:textarea>
								</smart:gridColumn>
							</smart:gridRow>
						</c:if>
	...
	<smart:scriptHead models="table,form,layer,element,laydate,upload">
		...
		<smart:buttonScriptAction>
			pass : function() {
				...
				smart.confirm({
					...
					<c:if test="${d.areaType eq '2' }">
						url:'ofcflow/zrtlbInto/operationFlowOuter',
					</c:if>
			...
			download : function() {
	            ...
	            $eleForm.attr("action","ofcflow/zrtlbInto/printIntroduction?id=${d.id}&type=1");
