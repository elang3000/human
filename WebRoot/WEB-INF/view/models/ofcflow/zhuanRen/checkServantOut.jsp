		...
		<smart:buttonScriptAction>
			checkServant : function(data) {
				...
				if(name&&name!=""&&cardNo&&cardNo!=""){
					var requestConfig = {};
					requestConfig.url = "ofcflow/zrtlbOut/checkServant";
			...
			,save : function() {
				if($("#status").val()=="1"){
					...
					smart.message({
						message : "本单位中不存在该公务员，无法发起转出申请！"
						,type : 'W' //S保存  I问号  W感叹号 E错误
					});
				}else if($("#status").val()=="4"){
					smart.message({
						message : "该人员正在转出流程中，无法再次发起转出申请！"
						,type : 'W' //S保存  I问号  W感叹号 E错误
					});
				}else{
					...
					window.location.href="ofcflow/zrtlbOut/edit?servantId="+id+"&id="+zhuanRenId;
				}
			}
		</smart:buttonScriptAction>
	</smart:scriptHead>
