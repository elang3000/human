...
<html>
<head>
<smart:initHead title="信息维护--人员检索" />
</head>
<smart:body>
	<smart:grid>
		<smart:card>
			...
			<smart:cardBody>
				<smart:form action="ofcflow/zrtlbInto/edit">
					<smart:fromTokenTag/>
					...
					<smart:accordionPanel id="test" isAccordion="true">
						<smart:accordionPanelItem title="人员检索" isShow="true">
							<smart:gridRow>
								...
								<smart:gridColumn colPart="4">
									<smart:singleSelect isNotNull="true" labelName="人员类型：" id="sourceType" data="${typeList}" display="block"></smart:singleSelect>
								</smart:gridColumn>
							</smart:gridRow>
							<smart:gridRow>
								<smart:gridColumn colPart="12">
									<smart:textInput labelName="原工作单位：" otherAttr="readonly" name="departName" id="departName"></smart:textInput>
								</smart:gridColumn>
							</smart:gridRow>
		...
		<smart:buttonScriptAction>
			checkServant : function(data) {
				...
				if(name&&name!=""&&cardNo&&cardNo!=""){
					var requestConfig = {};
					requestConfig.url = "ofcflow/diaoren/checkServant";
					requestConfig.data = {name:name,cardNo:cardNo,type:sourceType};
					requestConfig.success = function(data){
						...
						if (data.success) {
							...
							if(code=="1"){//人员库中检索出的数据，点击提交后不可编辑
								...
								$("#status").val("4");
							}else if(code=="5"){//该人员已在本单位，不能发起调任申请！
								$("#status").val("5");
							}
						}
			...
			,save : function() {
				if($("#status").val()=="1"){
					...
					smart.message({
						message : "系统中不存在该人员，无法发起调任申请！"
						,type : 'W' //S保存  I问号  W感叹号 E错误
					});
				}else if($("#status").val()=="4"){
					smart.message({
						message : "该人员正在调任流程中，无法再次发起调任申请！"
						,type : 'W' //S保存  I问号  W感叹号 E错误
					});
				}else if($("#status").val()=="5"){
					smart.message({
						message : "该人员已在本单位，无法发起调任申请！"
						,type : 'W' //S保存  I问号  W感叹号 E错误
					});
				}else{
					...
					var sourceType = $("#sourceType").val();
					window.location.href="ofcflow/diaoren/edit?servantId="+id+"&name="+name+"&cardNo="+cardNo+"&id="+zhuanRenId+"&type="+sourceType;
				}
			}
		</smart:buttonScriptAction>
	</smart:scriptHead>
