...
<smart:body>
	<smart:grid>
		<smart:card>
			...
			<smart:cardBody>
				<smart:gridRow>
					<smart:title title="申请单位编制信息" style="margin-top: 5px;" color="blue" />
				</smart:gridRow>
		...
		<smart:tableScriptAction tableId="navigationList" checkbox="true"
			...
			edit : function(data) {
				smart.show({
					title : '编辑转入信息',
					size : 'full',
					url : "ofcflow/exchangeB/servantView?isFlow=${isFlow}&status=${d.status}&areaType=${d.areaType}&id="+data.data.id,
					scrollbar : false
				});
			}
			,
			view : function(data) {
				smart.show({
					title : '查看转入信息',
					size : 'full',
					url : "ofcflow/exchangeB/servantView?status=${d.status}&areaType=${d.areaType}&id="+data.data.id,
					scrollbar : false
				});
			}
			,reload : function(){
		...
		<smart:buttonScriptAction>
			download : function(data) {
	           //通过form下载
	            ...
	            $eleForm.attr("action","ofcflow/material/downLoad?id=${itemids}&name=${introductionName}");
