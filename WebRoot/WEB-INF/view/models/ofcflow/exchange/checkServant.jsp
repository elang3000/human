...
<html>
<head>
<smart:initHead title="信息维护--参公交流" />
</head>
<smart:body>
	<smart:grid>
		<smart:card>
			...
			<smart:cardBody>
				<smart:form>
					...
					<smart:textInput type="hidden" name="id" id="exchangeId"></smart:textInput>
						...
						<smart:accordionPanelItem title="人员检索" isShow="true">
							...
							<smart:gridRow>
								<smart:gridColumn colPart="12">
									<smart:textInput labelName="现工作单位：" otherAttr="readonly" name="departName" id="departName"></smart:textInput>
								</smart:gridColumn>
							</smart:gridRow>
		...
		<smart:buttonScriptAction>
			checkServant : function(data) {
				...
				if(name&&name!=""&&cardNo&&cardNo!=""){
					var requestConfig = {};
					requestConfig.url = "ofcflow/exchange/checkServant";
					...
					requestConfig.success = function(data){
						...
						if (data.success) {
							...
							}else if(code=="2"){//转任信息中检索出的数据，点击后可编辑
								$("#exchangeId").val(data.data.id);
								if(data.data.departname){
									$("#departName").val(data.data.departname);
								}
							}else if(code=="3"){//正在转任流程
								$("#status").val("4");
							}
						}
			...
			,save : function() {
				if($("#status").val()=="1"){
					...
					smart.message({
						message : "系统中不存在该参公人员，无法发起转任申请！"
						,type : 'W' //S保存  I问号  W感叹号 E错误
					});
				}else if($("#status").val()=="4"){
					smart.message({
						message : "该人员正在转任流程中，无法再次发起转任申请！"
						,type : 'W' //S保存  I问号  W感叹号 E错误
					});
				}else{
					...
					window.location.href="ofcflow/exchange/edit?servantId="+id+"&name="+name+"&cardNo="+cardNo+"&id="+exchangeId;
				}
			}
		</smart:buttonScriptAction>
	</smart:scriptHead>
