			...
			<smart:cardBody>
				...
				<smart:gridRow colSpace="5">
					<smart:gridColumn colPart="12" deviceType="md">
						<smart:table id="navigationList" url="ofcflow/exchangeOut/infoList"
							...
							<script type="text/html" id="navListToolBar">
										{{#  if(d.status=="0"){ }}
										<shiro:hasPermission name="EDIT_EXCHANGE_OUT_BTN">
											<a class="layui-btn layui-btn-xs layui-btn-default" lay-event="edit"  title="编辑">
												<i class="fa fa-edit"></i>
											</a>
										</shiro:hasPermission>
										<shiro:hasPermission name="REMOVE_EXCHANGE_OUT_BTN">
		...
		<smart:tableScriptAction tableId="navigationList" checkbox="true"
			...
			edit : function(data) {
				smart.show({
					title : '调出管理',
					size : 'full',
					url : "ofcflow/exchangeOut/edit?id="+data.data.id,
					scrollbar : false
				});
			}
			,flow : function(data) {
				smart.show({
					title : '调出管理',
					size : 'full',
					url : "ofcflow/exchangeOut/toFlow?id="+data.data.id,
					scrollbar : false
				});
			}
			,view : function(data) {
				smart.show({
					title : '调出管理',
					size : 'full',
					url : "ofcflow/exchangeOut/toView?id="+data.data.id,
					scrollbar : false
				});
			}
			,delete : function(data) {
				smart.confirm({
					title:'提示',
					message:'您确定要删除这条记录？',
					url:'ofcflow/exchangeOut/delete',
