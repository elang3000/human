		...
		<smart:card>
			...
			<smart:cardBody>
				<smart:gridRow>
					<smart:tabPanelParent filter="tab"
						...
						<smart:tabPanel>
							...
							<smart:tabPanelItem turnurl="ofcflow/train/flow" show="false" eId="" itemName="流程审批"></smart:tabPanelItem>
						</smart:tabPanel>
					...
					<smart:fieldSet title="条件查询" style="margin-top: 5px;" color="blue">
						<smart:form>
							<smart:gridColumn colPart="4">
								<smart:textInput labelName="培训名称：" placeholder="请输入培训名称" name="trainName"></smart:textInput>
							</smart:gridColumn>
							<smart:gridColumn colPart="4">
								<smart:textInput labelName="开始时间：" placeholder="请输入开始时间" name="startDate" id="startDate"></smart:textInput>
							</smart:gridColumn>
							<smart:gridColumn colPart="4">
								<smart:textInput labelName="结束时间：" placeholder="请输入结束时间" name="endDate" id="endDate"></smart:textInput>
							</smart:gridColumn>
							<smart:gridColumn colPart="4">
								<smart:singleSelect display="block" isAddDefaltOption="true" labelName="年度培训考核："
									name="yearPlan.id" url="ofcflow/train/year/find"></smart:singleSelect>
							</smart:gridColumn>
							<smart:gridColumn colPart="6" colOffset="1">
								<smart:buttonGroup container="true">
									...
									<smart:button size="sm" title="重置"
		   							...
		   							<smart:button size="sm" method="add" title="新增单位培训考核">
										<smart:icon icon="plus">&nbsp;新增单位培训考核</smart:icon>
									</smart:button>
									</shiro:hasPermission>
									<shiro:hasPermission name="MANAGE_TRAIN_YEAR_PLAN_BTN">
									<smart:button theme="danger" size="sm" method="showPlanView"
										title="管理年度培训考核">
										<smart:icon icon="gears">&nbsp;管理年度培训考核</smart:icon>
									</smart:button>
									</shiro:hasPermission>
								</smart:buttonGroup>
							</smart:gridColumn>
				...
				<smart:gridRow colSpace="5">
					<smart:gridColumn>
						<smart:table id="navigationList" url="ofcflow/train/pageList" sortField="name" sortType="asc" 
							...
							<tr>
								...
								<smart:tableItem field="description" width=".2" sort="false">培训描述</smart:tableItem>
								<smart:tableItem field="status" width=".15" sort="false">状态</smart:tableItem>
								<smart:tableItem align="center" width=".20" fixed="right" unresize="true" toolbar="navListToolBar">操作</smart:tableItem>
							</tr>
							<script type="text/html" id="navListToolBar">
										...
										<shiro:hasPermission name="UPLOAD_TRAIN_PLAN_BTN">
										<a class="layui-btn layui-btn-xs layui-btn-warm" lay-event="upload"  title="上报">
											<i class="fa fa-upload"></i>
										</a>
										</shiro:hasPermission>
										<shiro:hasPermission name="ADD_TRAIN_PERSON_BTN">
										<a class="layui-btn layui-btn-xs layui-btn-normal" lay-event="addUser"  title="添加人员">
		...
		<smart:tableScriptAction tableId="navigationList" checkbox="true"
			sort="true" rowEdit="true">
				edit : function(data) {
					...
					title : '编辑培训信息',
					size : 'full',
					url : 'ofcflow/train//trainPlan/plan?trainPlanId='+data.data.id,
					scrollbar : false,
					});
				},
				upload : function(data) {
					...
					title : '培训学时上报',
					size : 'full',
					url : 'ofcflow/train/uploadTrainPlan?id='+data.data.id,
					scrollbar : false,
					});
				},
				addUser : function(data) {
					...
					title : '培训学时人员添加',
					size : 'full',
					url : 'ofcflow/train/addTrainPerson?id='+data.data.id,
					scrollbar : false,
					});
				},
		...
		<smart:buttonScriptAction>
			...
			add : function() {
				smart.show({
					title : '新增培训信息',
					size : 'full',
					url : "ofcflow/train/pre",
					size:'lg',
					scrollbar : false
				});
			},
			showPlanView : function(data) {
				 window.location.href="ofcflow/train/year/list";
			}
		 </smart:buttonScriptAction>
	</smart:scriptHead>
</smart:body>
