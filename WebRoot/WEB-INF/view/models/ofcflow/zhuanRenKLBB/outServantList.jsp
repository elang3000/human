			...
			<smart:cardBody>
				...
				<smart:gridRow colSpace="5">
					<smart:gridColumn colPart="12" deviceType="md">
						...
						<smart:table id="navigationList" url="ofcflow/zrklbOutB/outServantList?id=${d.id}" text="未获取到数据！">
							<tr>
								...
								<smart:tableItem field="sourceOrgan" width=".15" sort="false">转出单位</smart:tableItem>
		...
		<smart:tableScriptAction tableId="navigationList" checkbox="true"
			...
			edit : function(data) {
				smart.show({
					title : '编辑转出信息',
					size : 'full',
					url : "ofcflow/zrklbOutB/servantEdit?id="+data.data.id,
					scrollbar : false
				});
			}
			,delete : function(data) {
				smart.confirm({
					title:'提示',
					message:'您确定要删除这条记录？',
					url:'ofcflow/zrklbOutB/servantDelete',
			...
			,view : function(data) {
				smart.show({
					title : '查看转出信息',
					size : 'full',
					url : "ofcflow/zrklbOutB/servantView?status=${d.status}&id="+data.data.id,
					scrollbar : false
				});
			}
			,reload : function(){
		...
		<smart:buttonScriptAction>
			...
			selectPerson : function() {
			    smart.show({
					title : '选择人员',
					size : 'full',
					url : "orgInfo/selectServants?busClass=com.wondersgroup.human.bo.ofcflow.ZhuanRenKLBOut&createOrganNodeId=${d.sourceOrgan.id}&ids="
					+$("#servantIds").val()+"&names="+$("#servantNames").val()+"&busParentClass=zhuanRenKLBOutBatch",
					scrollbar : false,
				});
			}
			,addPerson : function(data) {
				smart.show({
					title : '新增转任人员',
					size : 'full',
					url : "ofcflow/zrklbOutB/servantEdit?batchId=${d.id}",
					scrollbar : false
				});
			}
			,goBack : function() {
		...
		form.on('submit(save)', function (data) {//表单保存
 		...
 		form.on('submit(submit)', function (data) {//表单提交
 			...
 			smart.confirm({
				title:'保存转任信息',
				message:'确认提交保存吗？',
				url:'ofcflow/zrklbOutB/operationFlow',
				params : smart.json("#editForm"),
				callback : function(){
					parent.layui.table.reload('navigationList');
					var index=parent.layer.getFrameIndex(window.name);
        ...
        setInterval(function(){
		    if($('#servantIds').val()!=""){
		    	if($('#servantIds').val()==""){
			 	...
			 	var requestConfig = {};
				requestConfig.url = "ofcflow/zrklbOutB/savePeople";
				...
				requestConfig.success = function(d){
					if(d.success){
						parent.layui.table.reload('navigationList');
						window.location.href="ofcflow/zrklbOutB/servantList?id="+d.data;//新增成功刷新此页面
					}else{
						smart.message({
							message : d.message,
							type : 'W' //S保存  I问号  W感叹号 E错误
