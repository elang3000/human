					...
					<smart:gridColumn colPart="12" deviceType="md">
						<smart:buttonGroup container="true" align="right">
							<c:if test="${d.areaType=='2'}">
									<smart:button method="addPerson" size="sm"
										title="新增转任人员" theme="normal">
						...
						url="ofcflow/zrklbIntoB/intoServantList?id=${d.id}" text="未获取到数据！">
							<tr>
		...
		<smart:tableScriptAction tableId="navigationList" checkbox="true"
			...
			edit : function(data) {
				smart.show({
					title : '编辑转入信息',
					size : 'full',
					url : "ofcflow/zrklbIntoB/servantEdit?id="+data.data.id,
					scrollbar : false
				});
			}
			,delete : function(data) {
				smart.confirm({
					title:'提示',
					message:'您确定要删除这条记录？',
					url:'ofcflow/zrklbIntoB/servantDelete',
			...
			,view : function(data) {
				smart.show({
					title : '查看转入信息',
					size : 'full',
					url : "ofcflow/zrklbIntoB/servantView?areaType=${d.areaType}&id="+data.data.id,
					scrollbar : false
				});
			}
			,reload : function(){
		...
		<smart:buttonScriptAction>
			...
			selectPerson : function() {
			    smart.show({
					title : '选择人员',
					size : 'full',
					url : "orgInfo/selectServants?busClass=com.wondersgroup.human.bo.ofcflow.ZhuanRenKLBInto&createOrganNodeId=${d.sourceOrgan.id}&ids="
					+$("#servantIds").val()+"&names="+$("#servantNames").val()+"&busParentClass=zhuanRenKLBIntoBatch",
					scrollbar : false,
				});
			}
			,addPerson : function(data) {
				smart.show({
					title : '新增转任人员',
					size : 'full',
					url : "ofcflow/zrklbIntoB/servantEdit?batchId=${d.id}",
					scrollbar : false
				});
			}
			,goBack : function() {
 		...
 		form.on('submit(submit)', function (data) {//表单提交
 			...
 			smart.confirm({
				...
				<c:if test="${d.areaType eq '1' }">
					url:'ofcflow/zrklbIntoB/operationFlow',
				</c:if>
				<c:if test="${d.areaType eq '2' }">
					url:'ofcflow/zrklbIntoB/operationFlowOuter',
				</c:if>
				params : smart.json("#editForm"),
				callback : function(){
					parent.layui.table.reload('navigationList');
        ...
        setInterval(function(){
		    if($('#servantIds').val()!=""){
		    	if($('#servantIds').val()==""){
			 	...
			 	var requestConfig = {};
				requestConfig.url = "ofcflow/zrklbIntoB/savePeople";
