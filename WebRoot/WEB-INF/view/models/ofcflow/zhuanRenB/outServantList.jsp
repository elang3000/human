			...
			<smart:cardBody>
				...
				<smart:gridRow colSpace="5">
					<smart:gridColumn colPart="12" deviceType="md">
						...
						<smart:table id="navigationList" url="ofcflow/zrtlbOutB/outServantList?id=${d.id}" text="未获取到数据！">
							<tr>
		...
		<smart:tableScriptAction tableId="navigationList" checkbox="true"
			...
			edit : function(data) {
				smart.show({
					title : '编辑转出信息',
					size : 'full',
					url : "ofcflow/zrtlbOutB/servantEdit?id="+data.data.id,
					scrollbar : false
				});
			}
			,delete : function(data) {
				smart.confirm({
					title:'提示',
					message:'您确定要删除这条记录？',
					url:'ofcflow/zrtlbOutB/servantDelete',
			...
			,view : function(data) {
				smart.show({
					title : '查看转出信息',
					size : 'full',
					url : "ofcflow/zrtlbOutB/servantView?status=${d.status}&id="+data.data.id,
					scrollbar : false
				});
			}
			,reload : function(){
		...
		<smart:buttonScriptAction>
			...
			selectPerson : function() {
			    smart.show({
					title : '选择人员',
					size : 'full',
					url : "orgInfo/selectServants?busClass=com.wondersgroup.human.bo.ofcflow.ZhuanRenTLBOut&createOrganNodeId=${d.sourceOrgan.id}&ids="
					+$("#servantIds").val()+"&names="+$("#servantNames").val()+"&busParentClass=zhuanRenTLBOutBatch",
					scrollbar : false,
				});
			}
			,addPerson : function(data) {
				smart.show({
					title : '新增转任人员',
					size : 'full',
					url : "ofcflow/zrtlbOutB/servantEdit?batchId=${d.id}",
					scrollbar : false
				});
			}
			,goBack : function() {
        ...
        setInterval(function(){
		    if($('#servantIds').val()!=""){
		    	if($('#servantIds').val()==""){
			 	...
			 	var requestConfig = {};
				requestConfig.url = "ofcflow/zrtlbOutB/savePeople";
				...
				requestConfig.success = function(d){
					if(d.success){
						parent.layui.table.reload('navigationList');
						window.location.href="ofcflow/zrtlbOutB/servantList?id="+d.data;//新增成功刷新此页面
					}else{
						smart.message({
							message : d.message,
							type : 'W' //S保存  I问号  W感叹号 E错误
