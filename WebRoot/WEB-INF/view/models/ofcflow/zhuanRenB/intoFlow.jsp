			...
			<smart:cardBody>
				<smart:form id="editForm">
					...
					<c:if test="${status==6 &&isFlow}">
						...
						<smart:gridRow>
							<smart:gridColumn colPart="4">
								<smart:textInput type="hidden" name="zhuanRenTLBOut.id" value="${o.id}"></smart:textInput>
								<smart:infoShowerLabel infoname="调往单位名称" infovalue="${o.gotoUnitName}"></smart:infoShowerLabel>
							</smart:gridColumn>
							<smart:gridColumn colPart="4">
								<smart:singleSelect initSelectedKey="${o.category.id}" name="zhuanRenTLBOut.category.id"
							...
							<smart:gridColumn colPart="4">
								<smart:date labelName="调出本单位日期：" display="block" name="zhuanRenTLBOut.outDate" id="outDate" value="${o.outDate}"></smart:date>
							</smart:gridColumn>
						</smart:gridRow>
						<smart:gridRow>
							<smart:gridColumn colPart="4">
								<smart:singleSelect labelName="提出调动类型：" name="zhuanRenTLBOut.proposeType.id" display="block" url="dictquery/sub/code/DM039" isAddDefaltOption="true" initSelectedKey="${o.proposeType.id }"></smart:singleSelect>
							</smart:gridColumn>
							<smart:gridColumn colPart="8">
								<smart:continuousSelect labelName="调动原因：" inputName="zhuanRenTLBOut.reasonType.id" codeTypeCode="DM015" inputVal="${o.reasonType.id}" valType="ID"/>
							</smart:gridColumn>
						</smart:gridRow>
						<smart:gridRow>
							<smart:gridColumn colPart="8">
								<smart:textarea labelName="调出备注：" name="zhuanRenTLBOut.remark"  display="block">${o.remark}</smart:textarea>
							</smart:gridColumn>
						</smart:gridRow>
					</c:if>
		...
		form.on('submit(save)', function (data) {//表单保存
			$("#result").val("1");//审批通过
				smart.confirm({
					title:'确认审批通过',
					message:'确认审批通过吗？',
					url:'ofcflow/zrtlbIntoB/saveThis',
