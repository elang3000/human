			...
			<smart:cardBody>
				<smart:form id="editForm">
					...
					<smart:gridRow>
						...
						<smart:gridColumn colPart="4">
							<smart:infoShowerLabel infoname="转任事由" infovalue="${d.causeType.name}"></smart:infoShowerLabel>
						</smart:gridColumn>
					</smart:gridRow>
					<smart:gridRow>
						<smart:gridColumn colPart="12">
							<smart:infoShowerLabel infoname="具体事由" infovalue="${d.specificCause}"></smart:infoShowerLabel>
						</smart:gridColumn>
					</smart:gridRow>
					<smart:gridRow>
						<smart:gridColumn colPart="4">
						...
						<c:if test="${d.areaType eq '1' }">
							<smart:gridColumn colPart="4">
								<smart:infoShowerLabel infoname="转任后人员类别" infovalue="${d.personType.name}"></smart:infoShowerLabel>
							</smart:gridColumn>
						</c:if>
					</smart:gridRow>
					<smart:gridRow>
					...
					<c:if test="${isFlow}">
						<c:if test="${d.status==6 }">
							...
							<smart:gridRow>
								<smart:gridColumn colPart="4">
									<smart:textInput type="hidden" name="zhuanRenKLBOutMgr.id" value="${o.id}"></smart:textInput>
									<smart:infoShowerLabel infoname="调往单位名称" infovalue="${o.gotoUnitName}"></smart:infoShowerLabel>
								</smart:gridColumn>
								<smart:gridColumn colPart="4">
									<smart:singleSelect initSelectedKey="${o.category.id}" name="zhuanRenKLBOutMgr.category.id"
								...
								<smart:gridColumn colPart="4">
									<smart:date labelName="调出本单位日期：" display="block" name="zhuanRenKLBOutMgr.outDate" id="outDate" value="${o.outDate}"></smart:date>
								</smart:gridColumn>
							</smart:gridRow>
							<smart:gridRow>
								<smart:gridColumn colPart="4">
									<smart:singleSelect labelName="提出调动类型：" name="zhuanRenKLBOutMgr.proposeType.id" display="block" url="dictquery/sub/code/DM039" isAddDefaltOption="true" initSelectedKey="${o.proposeType.id }"></smart:singleSelect>
								</smart:gridColumn>
								<smart:gridColumn colPart="8">
									<smart:continuousSelect labelName="调动原因：" inputName="zhuanRenKLBOutMgr.reasonType.id" codeTypeCode="DM015" inputVal="${o.reasonType.id}" valType="ID"/>
								</smart:gridColumn>
							</smart:gridRow>
							<smart:gridRow>
								<smart:gridColumn colPart="8">
									<smart:textarea labelName="调出备注：" name="zhuanRenKLBOutMgr.remark"  display="block">${o.remark}</smart:textarea>
								</smart:gridColumn>
							</smart:gridRow>
						</c:if>
		...
		<smart:buttonScriptAction>
			pass : function() {
				...
				smart.confirm({
					...
					message:'确认审批通过吗？',
					<c:if test="${d.areaType eq '1' }">
						url:'ofcflow/zrklbInto/operationFlow',
					</c:if>
			...
			download : function() {
	            ...
	            $eleForm.attr("action","ofcflow/zrklbInto/printIntroduction?id=${d.id}&type=1");
