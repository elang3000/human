...
<smart:body>
	<smart:grid>
		<smart:card>
			...
			<smart:cardBody>
				<smart:form action="ofcflow/zrklbInto/edit">
					<smart:fromTokenTag/>
		...
		<smart:buttonScriptAction>
			checkServant : function(data) {
				...
				if(name&&name!=""&&cardNo&&cardNo!=""){
					var requestConfig = {};
					requestConfig.url = "ofcflow/zrklbInto/checkServant";
					...
					requestConfig.success = function(data){
						...
						if (data.success) {
							...
							if(code=="1"){//人员库中检索出的数据，点击提交后不可编辑
								...
								$("#status").val("4");
							}else if(code=="5"){//该人员已在本单位，不能发起转任申请！
								$("#status").val("5");
							}
						}
			...
			,save : function() {
				if($("#status").val()=="1"){
					...
					window.location.href="ofcflow/zrklbInto/edit?servantId="+id+"&name="+name+"&cardNo="+cardNo+"&id="+zhuanRenId;
				}
			}
		</smart:buttonScriptAction>
	</smart:scriptHead>
