						...
						<smart:gridColumn>
							<smart:grid>
								...
								<smart:gridRow>
									...
									<smart:gridColumn colPart="4">
										<smart:textInput labelName="主管（从事）工作：" shortName="主管工作" name="workContentRange" value="${post.workContentRange}" ></smart:textInput>
									</smart:gridColumn>
								</smart:gridRow>
								<smart:gridRow>
									<smart:gridColumn colPart="4">
									...
									<smart:gridColumn colPart="4">
										<smart:numberInput min="0" labelName="集体内排列顺序号:" shortName="集体内顺序号" name="collectiveNo" value="${post.collectiveNo}"  display="block" type="text"></smart:numberInput>
									</smart:gridColumn>
								</smart:gridRow>
								
								<smart:gridRow>
									<smart:gridColumn colPart="4">
										<smart:textInput labelName="决定或批准任职的文号：" shortName="批准任职文号" name="approvalNumber" value="${post.approvalNumber}" ></smart:textInput>
									</smart:gridColumn>
									<smart:gridColumn colPart="4">
										<smart:date labelName="决定或批准任职的日期：" shortName="批准任职日期" display="block" name="approvalDate" id="approvalDate" value="${post.approvalDate}"></smart:date>
									</smart:gridColumn>
								</smart:gridRow>
								<smart:gridRow>
								...
								<smart:gridRow>
									<smart:gridColumn colPart="4">
										<smart:singleSelect id="tenureStatusId" isAddDefaltOption="true" name="tenureStatus.id" labelName="任职状态：" display="block" url="dictquery/sub/code/DM007" initSelectedKey="${post.tenureStatus.id}" verify="required" isNotNull="true"></smart:singleSelect>
									</smart:gridColumn>
									<smart:gridColumn colPart="4">
											...
											function topChangeEvent(data){
												if(data.elem[data.elem.selectedIndex].text=="是"){
													layui.use('smart',function(){
														...
														smart=smart(); 
														var nowPost = $('#tenureStatusId').next().find('.layui-anim').find('dd.layui-this').text();
														if(nowPost != '在任'){
															smart.message({
																message : "最高职务必要条件为当前职务状态为在任",
										...
										<smart:gridColumn colPart="4">
											<smart:singleSelect labelName="是否兼任下级领导职务：" shortName="兼下级领导职务" id="isLowerLeader" name="isLowerLeader.id" display="block" url="dictquery/sub/code/DM215" isAddDefaltOption="true" initSelectedKey="${post.isLowerLeader.id }" verify="required" isNotNull="true"></smart:singleSelect>
										</smart:gridColumn>
																		
								</smart:gridRow>
									<smart:gridRow>
										<smart:gridColumn colPart="4">
											<smart:textInput labelName="决定或批准免职的文号：" shortName="批准免职文号" name="approvalDismissalNumber" value="${post.approvalDismissalNumber}" ></smart:textInput>
										</smart:gridColumn>
										<smart:gridColumn colPart="4">
											<smart:date labelName="决定或批准免职的日期：" shortName="批准免职日期"  display="block" name="approvalDismissalDate" id="approvalDismissalDate" value="${post.approvalDismissalDate}"></smart:date>
										</smart:gridColumn>
									</smart:gridRow>	
									<smart:gridRow>
										<smart:gridColumn colPart="8">
											<smart:continuousSelect id="dismissalReason" labelName="免职原因类别：" inputName="dismissalReason.id" codeTypeCode="DM009" inputVal="${post.dismissalReason.id}" valType="ID" widthPercent="0.3333"/>
										</smart:gridColumn>
									</smart:gridRow>		
							</smart:grid>
						</smart:gridColumn>
