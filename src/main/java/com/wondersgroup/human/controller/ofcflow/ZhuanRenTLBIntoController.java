...
public class ZhuanRenTLBIntoController extends GenericController implements ApplicationContextAware{
	@Autowired
...
//			zhuanRenTLBIntoMgrService.remove(id);
	...
	public AjaxResult operationFlow(ZhuanRenTLBIntoMgr temp, HttpServletRequest request) {
		...
		try {
			...
			if(ZhuanRenTLBIntoMgr.STATUS_ZHUANREN_PRINT==temp.getStatus()&&FlowRecord.PASS.equals(r)){
				//发送通知
