 ...
 */
@Repository
public class SwDegreeRepositoryImpl extends GenericRepositoryImpl<SrDegree> implements SwDegreeRepository {
	
	public Class<SrDegree> getEntityClass() {
		
		return SrDegree.class;
	}
	
	@Override
	public void updateAllDegreeTopTipBySid(String servantId, CodeInfo codeInfo) {
		
		Query query = this.currentSession().createQuery("update com.wondersgroup.human.bo.socialworker.SrDegree set topFlag = :topFlag where socialWorker.id = :servantId");
