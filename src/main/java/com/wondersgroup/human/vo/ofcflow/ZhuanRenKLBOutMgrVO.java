 ...
 * @since     [产品/模块版本]
 */
public class ZhuanRenKLBOutMgrVO {
	...
	public ZhuanRenKLBOutMgrVO() {
		
	}
	
	public ZhuanRenKLBOutMgrVO(ZhuanRenKLBOutMgr d) {
		...
		if(d.getStatus() == ZhuanRenKLBOutMgr.STATUS_ZHUANCHU_CONFIRM){
			...
			if(organ!=null&&d.getSourceOrgan()!=null){
				if(!organ.getId().equals(d.getSourceOrgan().getId())){//如果当前登录人所在单位不是转出单位，只有查看权限
					...
					this.status = ZhuanRenKLBOutMgr.STATUS_ZHUANCHU_CONFIRM+"";
				}
			}else{
				this.status = "3";
			}
	...
	public String convertState(int state) {
		...
		if (state == ZhuanRenKLBOutMgr.STATUS_ZHUANCHU_STATE) {
			return "待发起转出流程";
		} else if (state == ZhuanRenKLBOutMgr.STATUS_ZHUANCHU_CONFIRM) {
			return "待转出单位备案";
		} else if (state == ZhuanRenKLBOutMgr.STATUS_ZHUANCHU_FINISH) {
			return "人员转出信息已备案";
		} else {
			return "";
		}
