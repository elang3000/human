...
/**
 * @ClassName: ZhuanRenTLBBatchVO
 * @Description: 同类别转任 批量数据表 VO
 * @author: tzy
 ...
 * @since     [产品/模块版本]
 */
public class ZhuanRenTLBIntoBatchVO {
	...
	public ZhuanRenTLBIntoBatchVO() {
		
	}
	
	public ZhuanRenTLBIntoBatchVO(ZhuanRenTLBIntoBatch z) {
		
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		this.id = z.getId();
		if (ZhuanRenTLBIntoBatch.AREA_THIS.equals(z.getAreaType())) {
			this.areaType = "本区转任";
		} else if (ZhuanRenTLBIntoBatch.AREA_OUTER.equals(z.getAreaType())) {
			this.areaType = "外区转任";
		}
	...
	public String convertState(int state) {
		...
		if (state == ZhuanRenTLBIntoBatch.STATUS_ZHUANREN_STATE) {
			return "待提交转任申请";
		} else if (state == ZhuanRenTLBIntoBatch.STATUS_ZHUANREN_TRIAL) {
			return "待上级单位审核";
		} else if (state == ZhuanRenTLBIntoBatch.STATUS_ZHUANREN_TRIAL_1) {
			return "待区人事主管部门一级审核";
		} else if (state == ZhuanRenTLBIntoBatch.STATUS_ZHUANREN_TRIAL_2) {
			return "待区人事主管部门二级审核";
		} else if (state == ZhuanRenTLBIntoBatch.STATUS_ZHUANREN_TRIAL_3) {
			return "待区人事主管部门三级审核";
		} else if (state == ZhuanRenTLBIntoBatch.STATUS_ZHUANREN_TRIAL_4) {
			return "待区人事主管部门四级审核";
		} else if (state == ZhuanRenTLBIntoBatch.STATUS_ZHUANREN_AGREE) {
			return "待转出单位同意";
		} else if (state == ZhuanRenTLBIntoBatch.STATUS_ZHUANREN_PRINT) {
			return "待区人事主管部门打印电子介绍信";
		} else if (state == ZhuanRenTLBIntoBatch.STATUS_ZHUANREN_FINISH) {
			return "人员信息已入库";
