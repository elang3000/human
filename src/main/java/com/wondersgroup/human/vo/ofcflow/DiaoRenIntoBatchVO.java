...
/**
 * @ClassName: DiaoRenIntoBatchVO
 * @Description: 公务员调任 批量数据表 VO
 * @author: tzy
 ...
 * @since     [产品/模块版本]
 */
public class DiaoRenIntoBatchVO {
	...
	public DiaoRenIntoBatchVO() {
		
	}
	
	public DiaoRenIntoBatchVO(DiaoRenIntoBatch z) {
		
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		this.id = z.getId();
		if (DiaoRenIntoBatch.AREA_THIS.equals(z.getAreaType())) {
			this.areaType = "本区转任";
		} else if (DiaoRenIntoBatch.AREA_OUTER.equals(z.getAreaType())) {
			this.areaType = "外区转任";
		}
		...
		if (z.getFlowRecord() != null && z.getStatus() == DiaoRenIntoBatch.STATUS_DIAOREN_STATE) {
			this.status = "99";
		} else {
	...
	public String convertState(int state) {
		...
		if (state == DiaoRenIntoBatch.STATUS_DIAOREN_STATE) {
			return "待提出调任申请";
		} else if (state == DiaoRenIntoBatch.STATUS_DIAOREN_TRIAL) {
			return "待上级单位审核";
		} else if (state == DiaoRenIntoBatch.STATUS_DIAOREN_TRIAL_1) {
			return "待区人事主管部门一级审核";
		} else if (state == DiaoRenIntoBatch.STATUS_DIAOREN_TRIAL_2) {
			return "待区人事主管部门二级审核";
		} else if (state == DiaoRenIntoBatch.STATUS_DIAOREN_TRIAL_3) {
			return "待区人事主管部门三级审核";
		} else if (state == DiaoRenIntoBatch.STATUS_DIAOREN_TRIAL_4) {
			return "待区人事主管部门四级审核";
		} else if (state == DiaoRenIntoBatch.STATUS_DIAOREN_AGREE) {
			return "待调出单位同意";
		} else if (state == DiaoRenIntoBatch.STATUS_DIAOREN_PRINT) {
			return "待区人事主管部门打印电子介绍信";
		} else if (state == DiaoRenIntoBatch.STATUS_DIAOREN_FINISH) {
			return "人员信息已入库";
