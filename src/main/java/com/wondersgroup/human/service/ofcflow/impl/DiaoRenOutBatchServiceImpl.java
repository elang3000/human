 ...
 * 
 * 文件名: DiaoRenOutBatchMgrServiceImpl.java 
 * 工程名: human
 * 包名: com.wondersgroup.human.service.ofcflow.impl 
 * 描述: 公务员调任 批量调出情况信息 服务实现类
 * 创建人: tzy   
...
/** 
 * @ClassName: DiaoRenOutBatchMgrServiceImpl 
 * @Description: 公务员调任 批量调出情况信息 服务实现类
 * @author: tzy
 ...
 */
@Service
public class DiaoRenOutBatchServiceImpl extends GenericServiceImpl<DiaoRenOutBatch> implements DiaoRenOutBatchService{
	 ...
	 */
	public Page<DiaoRenOutBatchVO> findbyHQLforVO(String hql,List<QueryParameter> listqueryparametry,Integer pageNo,Integer pagesize){
		Page<DiaoRenOutBatch> temppage=this.findByHQL(hql, listqueryparametry, pageNo, pagesize);
		List<DiaoRenOutBatchVO> voList = new ArrayList<>();
		for(DiaoRenOutBatch s:temppage.getResult()){
			DiaoRenOutBatchVO vo = new DiaoRenOutBatchVO(s);
			voList.add(vo);
		}
		Page<DiaoRenOutBatchVO> page = new Page<>(temppage.getStart(), temppage.getCurrentPageSize(), temppage.getTotalSize(), temppage.getPageSize(), voList);
		return page;
	}
	/**
	 * @Title: remove 
	...
	public void remove(String id){
		DetachedCriteria criteria = DetachedCriteria.forClass(DiaoRenOut.class);
		criteria.add(Restrictions.eq("diaoRenOutBatch.id", id));
		criteria.add(Restrictions.eq("removed", false));
		List<DiaoRenOut> list = diaoRenOutService.findByCriteria(criteria);
		for(DiaoRenOut t : list){
			diaoRenOutService.delete(t);
		}
		DiaoRenOutBatch z = get(id);
		delete(z);
	}
	
	/**
	 * @Title: savePeople 
	 * @Description: 添加调任人员信息
	 * @param id
	 * @param servantIds	人员ID，逗号分隔
	 * @return: void
	 */
	public void savePeople(DiaoRenOutBatch z,String servantIds){
		if(StringUtils.isBlank(z.getId())){//如果没有保存批次信息，先保存
			z.setStatus(DiaoRenOutBatch.STATUS_DIAOCHU_STATE);//流程状态，待提交
			z.setAreaType(DiaoRenIntoBatch.AREA_OUTER);//在选择人员时才新增数据都是调出到外区
			z.setTargetType(DiaoRenIntoBatch.SOURCE_TYPE_1);//默认事业人员
			save(z);
		}else{
			z = get(z.getId());
		}
		...
		for(String i : idArr){
			...
			zz.setDiaoRenOutBatch(z);//关联批次信息
			zz.setServant(servant);//关联人员信息
			diaoRenOutService.save(zz);
		}
	}
	/**
	 * @Title: saveFlow 
	 ...
	 * @return: void
	 */
	public void saveFlow(DiaoRenOutBatch temp,String opinion,String r){
		...
		if(DiaoRenOutBatch.STATUS_DIAOCHU_STATE==temp.getStatus()){//提交环节，锁编
			executeLockOutFormationAndPost(temp);
		}
		if(DiaoRenOutBatch.STATUS_DIAOCHU_TRIAL==temp.getStatus()){//调出单位确认环节，校验数据是否填写
			//1.校验数据是否填写
			DetachedCriteria criteria = DetachedCriteria.forClass(DiaoRenOut.class);
			criteria.add(Restrictions.eq("diaoRenOutBatch.id", temp.getId()));
			criteria.add(Restrictions.eq("removed", false));
			List<DiaoRenOut> list = diaoRenOutService.findByCriteria(criteria);//该批次下所有调任人员信息，判断各个职级人数是否和批次中人数一致
			for(DiaoRenOut i : list){
		...
		FlowRecord flow;
		if(DiaoRenOutBatch.STATUS_DIAOCHU_STATE==temp.getStatus()&&temp.getFlowRecord()==null){//提交环节，生成流程数据
			
			flow = new FlowRecord();
			flow.setAppNodeId(appNode.getId());//流程业务所在系统
			flow.setBusId(temp.getId());//流程业务ID
			flow.setBusName("调出到外区");//流程业务名称
			flow.setBusType("DiaoRenOutMgr_OUTER");//流程业务类型
			flow.setTargetOrganNode(userOrg);//流程业务目标组织
			flow.setTargetSecurityUser(user);;//流程业务目标人员
			flow = workflowService.createFlowRecord(flow, "STATUS_DIAOCHU_OUTER_STATE",temp.getSourceOrgan());//初始节点
		}else{
			flow = temp.getFlowRecord();
			flow.setOpinion(opinion);
			flow.setResult(r);
			if(DiaoRenOutBatch.STATUS_DIAOCHU_STATE==temp.getStatus()){
				flow.setResult(FlowRecord.PASS);
				flow = workflowService.completeFlowRecordByAppoint(flow,temp.getSourceOrgan());//提交下个节点
			}else{
				if(DiaoRenOutBatch.STATUS_DIAOCHU_TRIAL_4==temp.getStatus()){
		...
		if(flow==null){
			...
			if(FlowRecord.PASS.equals(r)){//流程最后环节
				updateServant(temp);
				executeFinish(temp);
				temp.setStatus(DiaoRenOutBatch.STATUS_DIAOCHU_FINISH);
			}else if(FlowRecord.STOP.equals(r)){//流程被中止
				executeUnLockFormationAndPost(temp);//返编
				
				temp.setStatus(FlowRecord.BUS_STOP);
				
				String title = messageSource.getMessage("stopFlowTitle", new Object[]{"调出到外区"}, Locale.CHINESE);
				String content = messageSource.getMessage("stopFlowContent", new Object[]{"调出到外区"}, Locale.CHINESE);
				AnnouncementManger.send(new SystemAnnouncementEvent(new AnnouncementEventData(true, temp.getCreater(), title, content, "","调出到外区")));
			}
			
		}else{
			temp.setStatus(DiaoRenOutBatch.power.get(flow.getOperationCode()));//实际有权限的操作节点
			temp.setFlowRecord(flow);//修改当前业务的流程节点
			
			if(DiaoRenOutBatch.STATUS_DIAOCHU_STATE==temp.getStatus()&&FlowRecord.NOPASS.equals(r)){//如果流程退回到最初节点，解锁编制，每次提交重新锁编
	 ...
	 * @return: void
	 */
	public void executeFinish(DiaoRenOutBatch temp){
		DetachedCriteria d = DetachedCriteria.forClass(DiaoRenOut.class);
		d.add(Restrictions.eq("diaoRenOutBatch.id", temp.getId()));
		d.add(Restrictions.eq("removed", false));
		List<DiaoRenOut> intoList = diaoRenOutService.findByCriteria(d);
	 ...
	 * @return: void
	 */
	public void executeUnLockFormationAndPost(DiaoRenOutBatch temp){
	 ...
	 */
	public void executeLockOutFormationAndPost(DiaoRenOutBatch temp){
	 ...
	 * @return: void
	 */
	private void updateServant(DiaoRenOutBatch z){
		DetachedCriteria d = DetachedCriteria.forClass(DiaoRenOut.class);
		d.add(Restrictions.eq("diaoRenOutBatch.id", z.getId()));
		d.add(Restrictions.eq("removed", false));
		List<DiaoRenOut> list = diaoRenOutService.findByCriteria(d);
		for(DiaoRenOut temp : list){
			//修改原数据状态为调出
	...
	private void createIntroduction(String id){
		...
		DiaoRenOutBatch batch = get(id);
		
		DetachedCriteria d = DetachedCriteria.forClass(DiaoRenOut.class);
		d.add(Restrictions.eq("diaoRenOutBatch.id", id));
		d.add(Restrictions.eq("removed", false));
		...
		for(int i =0;i<list.size();i++){
			...
			try {
				...
				p[0] = into.getServant().getName();//调任人员名称
