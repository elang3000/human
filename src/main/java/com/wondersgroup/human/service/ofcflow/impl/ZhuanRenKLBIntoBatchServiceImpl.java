 ...
 * 
 * 文件名: ZhuanRenKLBBatchServiceImpl.java 
 * 工程名: human
 * 包名: com.wondersgroup.human.service.ofcflow.impl 
 * 描述: 跨类别转任 批量数据表 服务实现类
 * 创建人: tzy   
...
/** 
 * @ClassName: ZhuanRenKLBBatchServiceImpl 
 * @Description: 跨类别转任 批量数据表 服务实现类
 * @author: tzy
 ...
 */
@Service
public class ZhuanRenKLBIntoBatchServiceImpl extends GenericServiceImpl<ZhuanRenKLBIntoBatch> implements ZhuanRenKLBIntoBatchService{
	@Autowired
	 ...
	 * @param pagesize
	 * @return
	 * @return: Page<ZhuanRenKLBIntoBatchVO>
	 */
	public Page<ZhuanRenKLBIntoBatchVO> findbyHQLforVO(String hql,List<QueryParameter> listqueryparametry,Integer pageNo,Integer pagesize){
		Page<ZhuanRenKLBIntoBatch> temppage=this.findByHQL(hql, listqueryparametry, pageNo, pagesize);
		List<ZhuanRenKLBIntoBatchVO> voList = new ArrayList<>();
		for(ZhuanRenKLBIntoBatch s:temppage.getResult()){
			ZhuanRenKLBIntoBatchVO vo = new ZhuanRenKLBIntoBatchVO(s);
			voList.add(vo);
		}
		Page<ZhuanRenKLBIntoBatchVO> page = new Page<>(temppage.getStart(), temppage.getCurrentPageSize(), temppage.getTotalSize(), temppage.getPageSize(), voList);
		return page;
	}
	/**
	 * @Title: remove 
	...
	public void remove(String id){
		DetachedCriteria criteria = DetachedCriteria.forClass(ZhuanRenKLBInto.class);
		criteria.add(Restrictions.eq("zhuanRenKLBIntoBatch.id", id));
		criteria.add(Restrictions.eq("removed", false));
		List<ZhuanRenKLBInto> list = zhuanRenKLBIntoService.findByCriteria(criteria);
		for(ZhuanRenKLBInto t : list){
			zhuanRenKLBIntoService.delete(t);
		}
		ZhuanRenKLBIntoBatch z = get(id);
		delete(z);
	}
	...
	public void savePeople(String id,String servantIds){
		...
		for(String i : idArr){
			...
			ZhuanRenKLBInto zz = new ZhuanRenKLBInto();
			zz.setZhuanRenKLBIntoBatch(z);//关联批次信息
			zz.setServant(servant);//关联人员信息
			zz.setFormerUnitName(z.getSourceOrgan().getName());
			zhuanRenKLBIntoService.save(zz);
		}
	}
	/**
	 * @Title: checkNumber 
	 ...
	 * @return: void
	 */
	private void checkNumber(ZhuanRenKLBIntoBatch temp){
		DetachedCriteria criteria = DetachedCriteria.forClass(ZhuanRenKLBInto.class);
		criteria.add(Restrictions.eq("zhuanRenKLBIntoBatch.id", temp.getId()));
		criteria.add(Restrictions.eq("removed", false));
		List<ZhuanRenKLBInto> list = zhuanRenKLBIntoService.findByCriteria(criteria);//该批次下所有转任人员信息，判断各个职级人数是否和批次中人数一致
	 ...
	 * @return: void
	 */
	public void saveFlow(ZhuanRenKLBIntoBatch temp,String opinion,String r){
		...
		if(ZhuanRenKLBIntoBatch.STATUS_ZHUANREN_STATE==temp.getStatus()){//提交环节，校验转任人数是否正确，检验编制，锁编
			//1.校验转任人数是否正确
			checkNumber(temp);
			//2.校验编控，锁编，回写真实占编数据
		...
		FlowRecord flow;
		if(ZhuanRenKLBIntoBatch.STATUS_ZHUANREN_STATE==temp.getStatus()&&temp.getFlowRecord()==null){//提交环节，生成流程数据
			
			flow = new FlowRecord();
			flow.setAppNodeId(appNode.getId());//流程业务所在系统
			flow.setBusId(temp.getId());//流程业务ID
			flow.setBusName("跨类别转任");//流程业务名称
			...
			if(ZhuanRenKLBIntoBatch.STATUS_ZHUANREN_STATE==temp.getStatus()){
				r = FlowRecord.PASS;
			}
			...
			flow.setResult(r);
			if(ZhuanRenKLBIntoBatch.STATUS_ZHUANREN_TRIAL_4==temp.getStatus()&&FlowRecord.PASS.equals(r)){//区人事审核通过之后，生成转出单位待办信息，生成转出数据
			...
			}else if(ZhuanRenKLBIntoBatch.STATUS_ZHUANREN_PRINT==temp.getStatus()&&FlowRecord.PASS.equals(r)){//区人事主管部门打印电子介绍信之后，更新转入转出子集信息
		...
		if(flow==null){
			...
			if(FlowRecord.PASS.equals(r)){//流程最后环节
				temp.setStatus(ZhuanRenKLBIntoBatch.STATUS_ZHUANREN_FINISH);
				//发送通知
				sendNotice(temp);
			}else if(FlowRecord.STOP.equals(r)){//流程被中止
				temp.setStatus(FlowRecord.BUS_STOP);
				
				String title = messageSource.getMessage("stopFlowTitle", new Object[]{"跨类别转任"}, Locale.CHINESE);
				String content = messageSource.getMessage("stopFlowContent", new Object[]{"跨类别转任"}, Locale.CHINESE);
				AnnouncementManger.send(new SystemAnnouncementEvent(new AnnouncementEventData(true, temp.getCreater(), title, content, "","跨类别转任")));
			}
			
		}else{
			temp.setStatus(ZhuanRenKLBIntoBatch.power.get(flow.getOperationCode()));//实际有权限的操作节点
			temp.setFlowRecord(flow);//修改当前业务的流程节点
			
			if(ZhuanRenKLBIntoBatch.STATUS_ZHUANREN_STATE==temp.getStatus()&&FlowRecord.NOPASS.equals(r)){//如果流程退回到最初节点，解锁编制，每次提交重新锁编
	 ...
	 * @return: void
	 */
	public void saveFlowOuter(ZhuanRenKLBIntoBatch temp,String opinion,String r){
		SecurityUser user = userService.load(SecurityUtils.getUserId());//当前登录人
		OrganNode userOrg = OrganCacheProvider.getOrganNodeInGovNode(SecurityUtils.getUserId());//当前登录所在单位
		AppNode appNode = (AppNode)SecurityUtils.getSession().getAttribute("appNode");
		if(StringUtils.isBlank(temp.getId())){
		...
		if(ZhuanRenKLBIntoBatch.STATUS_ZHUANREN_STATE==temp.getStatus()){//提交环节，检验编制，锁编
			//1.校验转任人数是否正确
			checkNumber(temp);
			//2.校验编控，锁编，回写真实占编数据
		...
		FlowRecord flow;
		if(ZhuanRenKLBIntoBatch.STATUS_ZHUANREN_STATE==temp.getStatus()&&temp.getFlowRecord()==null){//提交环节，先生成流程数据
			...
			flow.setOpinion(opinion);
			flow.setResult(r);
			if(ZhuanRenKLBIntoBatch.STATUS_ZHUANREN_TRIAL_4==temp.getStatus()&&FlowRecord.PASS.equals(r)){//区人事审核通过之后，生成介绍信
				createIntroduction(temp.getId());//生成介绍信
			}else if(ZhuanRenKLBIntoBatch.STATUS_ZHUANREN_PRINT==temp.getStatus()&&FlowRecord.PASS.equals(r)){//区人事主管部门打印电子介绍信之后，更新转入转出子集信息
				createServant(temp);//更新人员基本信息
				//流程结束，改变编制
	 ...
	 * @return: void
	 */
	public void executeFinish(ZhuanRenKLBIntoBatch temp){
		DetachedCriteria d = DetachedCriteria.forClass(ZhuanRenKLBInto.class);
		d.add(Restrictions.eq("zhuanRenKLBIntoBatch.id", temp.getId()));
		d.add(Restrictions.eq("removed", false));
		List<ZhuanRenKLBInto> intoList = zhuanRenKLBIntoService.findByCriteria(d);

		if(ZhuanRenKLBIntoBatch.AREA_THIS.equals(temp.getAreaType())){//本区
		...
		for(ZhuanRenKLBInto z : intoList){
			if(ZhuanRenKLBIntoBatch.AREA_THIS.equals(temp.getAreaType())){//本区
...
//			formationControlService.executeIntoPost(temp.getTargetOrgan().getId(),tempJ.getCode().getCode(),temp.getIsLowToHigh());//3.增加调入单位实际职级数  职级save方法已增加
	 ...
	 * @return: void
	 */
	public void executeUnLockFormationAndPost(ZhuanRenKLBIntoBatch temp){
	 ...
	 */
	public List<JudgePostResult> executeValidateAndLockFormation(ZhuanRenKLBIntoBatch temp){
	 ...
	 * @return: void
	 */
	public void queryValidateFormationAndPostLvlNum(ZhuanRenKLBIntoBatch temp){
...
//		编控，校验编制数是否足够，判断数据能否保存，如果超编，抛出异常
	 ...
	 * @return: void
	 */
	private void sendNotice(ZhuanRenKLBIntoBatch temp){
		DetachedCriteria d = DetachedCriteria.forClass(ZhuanRenKLBInto.class);
		d.add(Restrictions.eq("zhuanRenKLBIntoBatch.id", temp.getId()));
		d.add(Restrictions.eq("removed", false));
		List<ZhuanRenKLBInto> list = zhuanRenKLBIntoService.findByCriteria(d);
		for(ZhuanRenKLBInto z : list){
			String name = z.getName();//外区转任人员
			//发送通知
			if(ZhuanRenKLBIntoBatch.AREA_THIS.equals(temp.getAreaType())){//本区转任人员
			...
			AnnouncementManger.send(new SystemAnnouncementEvent(new AnnouncementEventData(true, z.getCreater(), title, content, "","跨类别转任")));
		}
	}
	/**
	 * @Title: createServant
	 ...
	 * @return: void
	 */
	public void createServant(ZhuanRenKLBIntoBatch temp){
		DetachedCriteria d = DetachedCriteria.forClass(ZhuanRenKLBInto.class);
		d.add(Restrictions.eq("zhuanRenKLBIntoBatch.id", temp.getId()));
		d.add(Restrictions.eq("removed", false));
		List<ZhuanRenKLBInto> list = zhuanRenKLBIntoService.findByCriteria(d);
	...
	//		into.setEnterReason(z.getEnterReason());//进入本单位原因
			...
			if(ZhuanRenKLBIntoBatch.AREA_THIS.equals(temp.getAreaType())){//本区转任才新增一条转出子集
				OutMgr out = new OutMgr();//转出子集信息
				ZhuanRenKLBOut o = zhuanRenKLBOutService.get(z.getZhuanRenKLBOut().getId());
			...
			//进出管理
			ManagerRecordDTO dto = new ManagerRecordDTO(servant.getId(),ManagerRecord.HUMAN_KLZR);
	 ...
	 * @return: void
	 */
	private void createOut(ZhuanRenKLBIntoBatch temp){
		DetachedCriteria d = DetachedCriteria.forClass(ZhuanRenKLBInto.class);
		d.add(Restrictions.eq("zhuanRenKLBIntoBatch.id", temp.getId()));
		d.add(Restrictions.eq("removed", false));
		List<ZhuanRenKLBInto> list = zhuanRenKLBIntoService.findByCriteria(d);
		for(ZhuanRenKLBInto z : list){
			DetachedCriteria d2 = DetachedCriteria.forClass(ZhuanRenKLBOut.class);
			d2.add(Restrictions.eq("zhuanRenKLBInto", z));
			List<ZhuanRenKLBOut> list2 = zhuanRenKLBOutService.findByCriteria(d2);
			if(list2==null||list2.size()==0){
				ZhuanRenKLBOut out = new ZhuanRenKLBOut();
				out.setZhuanRenKLBInto(z);//关联转入数据
...
//				out.setReasonType(z.getEnterReason());//调动原因
				zhuanRenKLBOutService.save(out);
				ZhuanRenKLBInto zz = zhuanRenKLBIntoService.get(z.getId());
				zz.setZhuanRenKLBOut(out);
				zhuanRenKLBIntoService.saveOrUpdate(zz);
			}
		}
	}
	/**
	...
	private void createIntroduction(String id){
		...
		ZhuanRenKLBIntoBatch batch = get(id);
