...
import com.wondersgroup.human.repository.ofc.RegistrationRepository;
 ...
 */
@Service
public class RegistrationServiceImpl extends GenericServiceImpl<Registration> implements RegistrationService {
	 ...
	 */
	@Override
	public Page<RegistrationVO> getPage(List<Predicate> filter, Sorts sort, Integer page, Integer limit) {
		
		Page<Registration> registrationPage = registrationRepository.findByFilter(filter, sort, page, limit);
		List<RegistrationVO> voList = new ArrayList<>();
		for (Registration e : registrationPage.getResult()) {
			RegistrationVO vo = new RegistrationVO(e);
			voList.add(vo);
		}
